...
fun turnToNeg(Var(a)) = Var(a)
    | turnToNeg(Neg(Neg(Var(a)))) = Var(a)
    | turnToNeg(Neg(p)) = if numConnectives(p) = 0 then Neg(p) else turnToNeg(p)
    | turnToNeg(And(p,q)) = Or(turnToNeg(Neg(p)),turnToNeg(Neg(q)))
    | turnToNeg(Or(p,q)) = And(turnToNeg(Neg(p)),turnToNeg(Neg(q)));
