...
__host__ __device__ bool update_constraints(uint16_t* target, uint16_t other_cell) {
  ...
  *target = old_value & ~(1<<other_digit);
  ...
  if(*target != old_value) return true;
  else return false;
}
...
__global__ void kernel(board_t* boards) {
  ...
  while(__syncthreads_count(changed) > 0) {
    ...
    for(int other_col=0; other_col<BOARD_DIM; other_col++) {
      ...
      changed |= update_constraints(&board.cells[index], board.cells[other_index]);
    }
    ...
    for(int other_row=group_row; other_row<group_row+GROUP_DIM; other_row++) {
      for(int other_col=group_col; other_col<group_col+GROUP_DIM; other_col++) {
